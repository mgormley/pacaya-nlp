...
public class SprlFactorGraphBuilderTest {
    ...
    private CPrm auxTest(TPrm tPrm) {
        ...
        srlFePrm.senseTemplates = Arrays
                .asList(new FeatTemplate1(Position.PARENT, PositionModifier.HEAD, TokProperty.LEMMA));
        ...
        AnnoSentenceCollection trainSents = new AnnoSentenceCollection(
                tPrm.trainSentences.stream().map(i -> sents.get(i)).collect(Collectors.toList()));
        CorpusStatistics cs = new CorpusStatistics(csPrm);
        cs.init(trainSents, false);
        cs.store = new AlphabetStore(trainSents);
        ...
        BiFunction<List<Integer>, Boolean, FgExampleList> makeExList = (sentIds, isTrain) -> exListBuilder
                .getInstance(new FgExampleList() {
                    ...
                    public LFgExample get(int i) {
                        AnnoSentence sent = sents.get(sentIds.get(i));
                        ...
                        if (isTrain) {
                            ...
                            return new UnlabeledFgExample(fg, ofc.getTemplates());

                        }
                    }
                });
        ofc.init(makeExList.apply(tPrm.trainSentences, true));
        FgExampleList testExamples = makeExList.apply(Arrays.asList(0), false);
        LFgExample ex = testExamples.get(0);
    ...
    private static class TPrm {
        ...
        List<Integer> trainSentences = Arrays.asList(0, 1);
        int testSentence = 0;
    }
    ...
    private static class EPrm {
        ...
        SrlFactorGraphBuilder srlBuilder;
        CorpusStatistics cs;
    }
    ...
    public void testSprlUnaries() throws IOException {
        ...
        CPrm cPrm = auxTest(new TPrm() {
            {
                sprlPairs = false;
                argTemplates = Arrays
                        .asList(new FeatTemplate1(Position.PARENT, PositionModifier.HEAD, TokProperty.CAPITALIZED));
                ...
                encode = p -> {
                    ...
                    p.sprlBuilder.build(p.isent, p.ofc, p.fg, p.cs);
        ...
        builder.build(cPrm.isent, ofc, fg, cPrm.cs);
    ...
    public void testSprlPairwise() throws IOException {
        CPrm cPrm = auxTest(new TPrm() {
            {
                sprlPairs = true;
                argTemplates = Arrays.asList(
                        new FeatTemplate1(Position.PARENT, PositionModifier.HEAD, TokProperty.CAPITALIZED),
        ...
        SprlFactorGraphBuilder builder = new SprlFactorGraphBuilder(cPrm.sprlPrm);
        ObsFeatureConjoiner ofc = cPrm.ofc;
        builder.build(cPrm.isent, cPrm.ofc, fg, cPrm.cs);
        assertEquals(4, builder.getSprlVars().length);
        assertEquals(4, builder.getSprlVars()[0].length);
        assertEquals(2, builder.getSprlVars()[0][0].length);
        assertEquals(2, builder.getSprlVars()[0][1].length);
        ...
        // 2 pairs each with 2 properties; so, 2 unary factors
        assertEquals(4, fg.getNumVars());
        // 2*2 + 2 = 6; 2 pairs each with 2 properties each with 1 unary plus 1
        // pairwise for each pair
        ...
        assertEquals(Arrays.asList(SprlFactorType.SPRL_UNARY, "awareness"), ofc.getTemplates().get(0).getKey());
        assertEquals(Arrays.asList(SprlFactorType.SPRL_UNARY, "volitional"), ofc.getTemplates().get(1).getKey());
        assertEquals(Arrays.asList(SprlFactorType.SPRL_PAIRWISE, "volitional", "awareness"),
                ofc.getTemplates().get(2).getKey());
        {
    ...
    public void testSprlSrlFactors() throws IOException {
        ...
        // 6 unary plus 4 role-spr pairwise plus two role,spr,spr three-way
        // factor
        ...
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "awareness"),
                ofc.getTemplates().get(3).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "volitional"),
                ofc.getTemplates().get(4).getKey());
        assertEquals(2, ofc.getTemplates().get(0).getNumConfigs());
    ...
    public void testSrlGivenSprlPlus() throws IOException {
        ...
        // role + role|aw=un + role|vol=un + role|vol=un&role|aw=un + role|aw=lk
        // + role|vol=lk + role|vol=lk&role|aw=lk
        // 7
        ...
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "GOLD_SPRL", "awareness",
                SprlLabelConverter.LIKELY), ofc.getTemplates().get(1).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "GOLD_SPRL", "volitional",
                SprlLabelConverter.LIKELY), ofc.getTemplates().get(2).getKey());
        assertEquals(
                Arrays.asList(JointFactorTemplate.ROLE_SPRL_SPRL, "GOLD_SPRL_PAIR", "volitional",
                        SprlLabelConverter.LIKELY, "awareness", SprlLabelConverter.LIKELY),
                ofc.getTemplates().get(3).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "GOLD_SPRL", "awareness",
                SprlLabelConverter.UNLIKELY), ofc.getTemplates().get(4).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "GOLD_SPRL", "volitional",
                SprlLabelConverter.UNLIKELY), ofc.getTemplates().get(5).getKey());
        assertEquals(
                Arrays.asList(JointFactorTemplate.ROLE_SPRL_SPRL, "GOLD_SPRL_PAIR", "volitional",
                        SprlLabelConverter.UNLIKELY, "awareness", SprlLabelConverter.UNLIKELY),
                ofc.getTemplates().get(6).getKey());
        for (int i = 0; i < 7; i++) {
            // argUnk, _, ARG0, ARG1, ARG1-to
    ...
    public void testSprlPlusGivenSrl() throws IOException {
        ...
        // 4 vars * (1 prop unary + 1 gold role unary) + 2 pairs * (1 spr
        // pairwise)
        assertEquals(10, fg.getNumFactors());
        assertEquals(10, fg.getFactors().size());

        // aw, vol, vol+aw, aware|role_sprl * 3 (observed), vol|role_sprl * 3
        // (observed in order observed)
        assertEquals(9, ofc.getTemplates().size());
        assertEquals(Arrays.asList(SprlFactorType.SPRL_UNARY, "awareness"), ofc.getTemplates().get(0).getKey());
        assertEquals(Arrays.asList(SprlFactorType.SPRL_UNARY, "volitional"), ofc.getTemplates().get(1).getKey());
        assertEquals(Arrays.asList(SprlFactorType.SPRL_PAIRWISE, "volitional", "awareness"),
                ofc.getTemplates().get(2).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "awareness", "GOLD_SRL", "ARG0-to"),
                ofc.getTemplates().get(3).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "volitional", "GOLD_SRL", "ARG0-to"),
                ofc.getTemplates().get(4).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "awareness", "GOLD_SRL", "ARG1"),
                ofc.getTemplates().get(5).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "volitional", "GOLD_SRL", "ARG1"),
                ofc.getTemplates().get(6).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "awareness", "GOLD_SRL", "ARG0"),
                ofc.getTemplates().get(7).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "volitional", "GOLD_SRL", "ARG0"),
                ofc.getTemplates().get(8).getKey());
        for (int i = 0; i < 7; i++) {
            if (i == 2) {
    ...
    public void testSrlGivenSprl() throws IOException {
        ...
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "GOLD_SPRL", "volitional",
                SprlLabelConverter.LIKELY), ofc.getTemplates().get(2).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "GOLD_SPRL", "awareness",
                SprlLabelConverter.UNLIKELY), ofc.getTemplates().get(3).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "GOLD_SPRL", "volitional",
                SprlLabelConverter.UNLIKELY), ofc.getTemplates().get(4).getKey());
        for (int i = 0; i < 5; i++) {
            // argUnk, _, ARG0, ARG1, ARG1-to
    ...
    public void testNeither() throws IOException {
        ...
        SrlFactorGraphBuilder srlBuilder = new SrlFactorGraphBuilder(cPrm.srlPrm);
        {
            ...
            FactorGraph fg = new FactorGraph();
            SprlFactorGraphBuilder.addSprlSrlFactors(sent, ofc, cPrm.cs, fg, null, null, cPrm.sprlPrm.pairwiseFactors);
        ...
        }
        {
            ...
            assertTrue(TestUtils.checkThrows(() -> SprlFactorGraphBuilder.addSprlSrlFactors(sent, ofc, cPrm.cs, fg,
                    builder, srlBuilder, sprlPrm.pairwiseFactors), IllegalArgumentException.class));
            assertEquals(0, fg.getNumVars());
            assertEquals(0, fg.getNumFactors());
    ...
    public void testAllPairsSprlGivenSrl() throws IOException {
        ...
        assertEquals(Arrays.asList(SprlFactorType.SPRL_UNARY, "awareness"), ofc.getTemplates().get(0).getKey());
        assertEquals(Arrays.asList(SprlFactorType.SPRL_UNARY, "volitional"), ofc.getTemplates().get(1).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "awareness", "GOLD_SRL", "_"),
                ofc.getTemplates().get(2).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "volitional", "GOLD_SRL", "_"),
                ofc.getTemplates().get(3).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "awareness", "GOLD_SRL", "ARG0-to"),
                ofc.getTemplates().get(4).getKey());
        assertEquals(Arrays.asList(JointFactorTemplate.ROLE_SPRL_BINARY, "volitional", "GOLD_SRL", "ARG0-to"),
    ...
    @Test
    public void testUnkFeatures() throws IOException {
        CPrm cPrm = auxTest(new TPrm() {
            {
                trainSentences = Arrays.asList(1);
                testSentence = 0;
                sprlPairs = true;
                argTemplates = Arrays
                        .asList(new FeatTemplate1(Position.PARENT, PositionModifier.HEAD, TokProperty.CAPITALIZED));
                sprlUnaryFactors = false;
                srlUnaryFactors = false;
                sprlRoleStructure = RoleStructure.PAIRS_GIVEN;
                srlRoleStructure = RoleStructure.PAIRS_GIVEN;
                sprlAllowSelfLoops = true;
                srlAllowSelfLoops = true;
        ...
        // 2 * 2 srlGsprl + 2 srlGsprl+
        assertEquals(6, fg.getNumFactors());

        BiasOnlyObsFeatureExtractor fe = new BiasOnlyObsFeatureExtractor(ofc, -1);
        // the first factor on the wants sleep pair should be unobserved
        assertEquals(6, fg.getFactors().size());
        for (int i = 0; i < 3; i++) {
            // 1 params for each of the unaries on the first (corresponding to
            // the bias only)
            FeatureVector fv = fe.calcObsFeatureVector((ObsFeExpFamFactor) fg.getFactor(i));
            assertEquals(1, fv.getNumImplicitEntries());
        }
        for (int i = 3; i < 6; i++) {
            // 0 params for each of the unaries on the second pred-arg pair
            // since the gold sprl values were unseen at train
            FeatureVector fv = fe.calcObsFeatureVector((ObsFeExpFamFactor) fg.getFactor(i));
            assertEquals(0, fv.getNumImplicitEntries());
        }
    }
